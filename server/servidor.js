...
app.get("/api/profissionais", async (req, res) => {
  try {
    ...
    // Verificar primeiro se a conexão está ok
    if (mongoose.connection.readyState !== 1) {
      console.log("⚠️ MongoDB não conectado, usando dados mock");
      return res.status(200).json(getProfissionaisMock());
    }

    // Primeiro, tenta buscar do MongoDB com timeout
    let profissionais = [];

    try {
      // Verifica se há profissionais no banco com timeout
      const count = await Profissional.countDocuments().maxTimeMS(15000);
      console.log(`📊 Profissionais no banco: ${count}`);

      if (count > 0) {
        // Busca profissionais reais com timeout
        ...
        profissionais = await criarProfissionaisExemplo();
      }
      ...
      const profissionaisFormatados = profissionais.map((prof) => {
        return {
          _id: prof._id,
          imagem: prof.foto || "/imagens/avatar-padrao.png", // Use sua imagem padrão
          nome: prof.nome || "Nome não informado",
          localizacao: prof.localizacao
            ? `${prof.localizacao.nome || ""} ${
                prof.localizacao.cidade || ""
              } ${prof.localizacao.estado || ""}`.trim()
            : "Local não informado",
      ...
      console.log("🔄 Usando dados mock devido a erro no banco");
      res.status(200).json(getProfissionaisMock());
    }
  } catch (error) {
