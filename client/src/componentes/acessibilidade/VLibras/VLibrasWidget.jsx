...
const VLibrasWidget = forwardRef(({ onStatusChange, isVisible: propIsVisible }, ref) => {
  ...
  const initializeVLibras = useCallback(() => {
    ...
    script.src = '/target/UnityLoader.js'; // Ensure this path is correct relative to your public directory
    script.async = true;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      setIsLoaded(false);
      setIsPlayerInitializing(false);
      // No need to update status on unmount unless parent handles it.
      // updateStatus('pending', 'VLibras descarregado.', 0); 
    };
